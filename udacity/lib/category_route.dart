...
class _CategoryRouteState extends State<CategoryRoute> {
  ...
  Future<void> _retrieveLocalCategories() async {
    ...
    data.forEach((key, value) {
      if (value is! List) {
        throw ('Data does not point to a List');
      }
      if (index > 6) {
        index %= 6;
      }
      final List<Unit> units = value.map<Unit>((dynamic unit) => Unit.fromJson(unit)).toList();
      final category = Category(name: key, color: _baseColors[index], units: units, iconLocation: _icons[index]);
      setState(() {
        if (index == 0) {
